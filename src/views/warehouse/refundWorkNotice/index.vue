<template>
  <div class="app-container">
    <el-form
      ...
      label-width="100px"
    >
      <el-form-item label="退货通知编号" prop="id">
        <el-input
          v-model="queryParams.id"
          placeholder="请输入退货通知编号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="质检通知编号" prop="noticeId">
        <el-input
          v-model="queryParams.noticeId"
          placeholder="请输入质检通知编号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          :disabled="single"
          @click="handleReturnComplete"
          v-hasPermi="['warehouse:refundWorkNotice:returnComplete']"
          >退货完成</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.refund_work_notice_status"
      ...
      <el-table-column label="操作" align="center" fixed="right" width="240">
        <template slot-scope="scope">
          <el-button
            ...
            @click="handleDetail(scope.row)"
            v-hasPermi="['warehouse:refundWorkNotice:detail']"
            >详情</el-button
          >
          <el-button
            size="mini"
            type="text"
            icon="el-icon-check"
            @click="handleManualWork(scope.row)"
            v-hasPermi="['warehouse:refundWorkNotice:manualWork']"
            >手动分配</el-button
          >
        </template>
      </el-table-column>
    ...
    <!-- 手动作业对话框 -->
    <work ref="work" @refreshTable="refreshTable" />
  </div>
</template>

<script>
import { listWorkNotice, returnComplete } from "@/api/warehouse/workNotice";
  ...
  methods: {
    ...
    handleReturnComplete() {
      ...
      this.$modal
        .confirm("是否完成退货作业？")
        .then(() => {
          returnComplete(rowData).then((response) => {
    ...
    /** 手动作业 */
    handleManualWork(row) {
      this.$refs.work.start(row);
    },
    /**作业取消 */
    handleWorkCancel(row) {
      this.$refs.workCancel.start(row);
    },
