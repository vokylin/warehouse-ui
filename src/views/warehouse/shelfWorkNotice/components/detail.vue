    ...
    getDetailList() {
      ...
      listWorkNoticeDetail(this.detailTableQueryParams).then((response) => {
        ...
        this.detailTableList.forEach((item) => {
          this.minStorageUnit.forEach((unit) => {
            if (item.locationId === unit.id) {
              item.locationName = unit.name;
            }
            if (item.sourceLocationId === unit.id) {
              item.sourceLocationName = unit.name;
            }
          });
        });
